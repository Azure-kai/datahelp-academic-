...
def ModuleUpgrade():
    ...
    模块更新方法
    -----------------------------
    0.打开保存库函数的目录,将最新版的数据复制并覆盖\n
    1.打开cmd\n
    2.先输入d:,将路径转移到d盘 \n
    3.输入模块所在的文件夹路径(cd D:\jjk\研究生\程序\AzureK) \n
...
def getAllFileName(FileDir, how = 'all'):
   ...
   2 
    ...
    if how == 'totalname':
        ...
        return [i.split('.')[0] for i in FileList]
    elif how == 'all':
        TotalFileName = ['\\'.join((FileDir,i)) for i in FileList]
...
def read_excel(file_path, collen = 6,  code_col = '股票代码', market_col = '', sheet_name = '', str_col = ['指数代码','股票代码','公告发布日期','交易日期']):
    '''
    函数功能
    -----------------------------
    ...
    def CodeCol_Check(df, code_col, collen, market_col):
        ...
        elif isinstance(collen, int):
            for index in range(len(df)):
                code = df.loc[index, code_col]
                df.loc[index, code_col] = '0'*(collen - len(code)) + df.loc[index, code_col]
        return df
    str_dict = {}
    for i in str_col:
        str_dict[i] = str
...
def baiduSentiment(data, CommentCol = '评论内容', client_id = 'jZgx0XTi08BKfLX0jNpmLea9', client_secret = '3f4AijnG0G59NTXCennNmuvUGZ7qM0Fu',decode = 'GBK'):
    ...
    for i in range(len(CommentContentList)):
        ...
        print('内容:', CommentContentList[i],'   极性:',sentiment)
...
def CAR(df, index_code, output = 'divided', estimated_period = [(-128,-8)], windows_period = (-5,5), CAR_interval = [(-5,5),(-4,4),(-3,3),(-2,2),(-1,1),(-1,0),(0,1)], date_title = '公告发布日期', 
        index_data_path = CAR_dict['mainland_index'], stock_data_path = CAR_dict['mainland_stock'], tradingday_data_path = CAR_dict['mainland_trading'], check_theshold = 0,
        index_col = ['交易日期','指数回报率'], stock_col = ['交易日期','日个股回报率'], tradingday_col = ['交易日期'], market_col = ''):
    '''
    函数功能
    -----------------------------
    异常收益率的计算
    ...
    def slice_list(date, date_list, interval, interval_element):
        ...
        if interval_element == 'int':
            ...
            print('element选项有误')
            return -1           
    def DateCheck(date, stock_trading_list, tradingday_list, estimated_period, windows_period, check_theshold):
        '''
        ...
        if len(slice_list(date, tradingday_list, estimated_period, interval_element = 'tuple').intersection(slice_list(date, stock_trading_list, estimated_period, interval_element = 'tuple'))) < len(slice_list(date, tradingday_list, estimated_period, interval_element = 'tuple'))-check_theshold:
    ...
    if index_data_path[-5:] == '.xlsx':
        index_data = local_read_excel(index_data_path, code_col = '指数代码')
    else:
        index_data = local_read_excel(''.join((index_data_path,index_code,'.xlsx')), code_col = '指数代码')
    ...
    for code in code_list:
        ...
        if stock_data_path[-5:] == '.xlsx':
            AllStockData = local_read_excel(stock_data_path, code_col = '股票代码')
            stock_data = AllStockData[AllStockData['股票代码']==code].copy().dropna(axis=0,subset=[stock_col[1]]).reset_index(drop=True)
        else:
            stock_data = local_read_excel(''.join((stock_data_path,code,'.xlsx')), code_col = '股票代码').dropna(axis=0,subset=[stock_col[1]]).reset_index(drop=True)#从数据库中读取相应的数据文件
        ...
        for index in range(len(notice_dataset)):
            ...
            if date in stock_trading_list and date in tradingday_list:
                if DateCheck(date, stock_trading_list, tradingday_list, estimated_period, windows_period, check_theshold) == True: #停牌检查无误后开始进行线性拟合
                    reason = '已计算'
                    ...
                    k,b = ARleastsq(data_x ,data_y)
                    '''根据计算出的k,b再计算窗口期内拟合收益率,进而得到异常收益率'''
                    windows_date = [str(i)[:date_len] for i in slice_list(date, stock_trading_list, windows_period, interval_element = 'int')]
                    ...
                    reason = DateCheck(date, stock_trading_list, tradingday_list, estimated_period, windows_period, check_theshold)
            else:
                reason = '该日期企业未进行股票交易'
            if reason != '已计算':
                k,b = '',''
    ...
    for market in list(Markets):
        ...
        if output == 'divided':
            CAR_eachMarket, Residue_eachMarket = CAR(newdf_eachMarket, index_code = index_dict[market], output = output, estimated_period = estimated_period , windows_period = windows_period, CAR_interval = CAR_interval, date_title = date_title, 
                                                     index_data_path = CAR_dict[market+'_index'], stock_data_path = CAR_dict[market+'_stock'], tradingday_data_path = CAR_dict[market+'_trading'], check_theshold = 0,
